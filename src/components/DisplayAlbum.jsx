...
const DisplayAlbum = () => {
    const { id } = useParams();
    const album = albumsData[id]; // Access the specific album
    ...
    return (
        <>
            ...
            <div className="mt-10 flex gap-8 flex-col md:flex-row md:items-end">
                <img className='w-48 rounded' src={album.image} alt={album.name || 'Album Cover'} />
            ...
            <hr />
            {
                songsData.map((item,index)=>(
                    <div key={index} className='grid grid-cols-3 sm:grid-cols-4 gap-2 p-2 items text-[]#a7a7a7 hover:bg-[#ffffff2b] cursor-pointer'>
